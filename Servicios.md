...
2. **análisis lingüístico del español** (puerto `8881`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

3. **análisis lingüístico del catalán** (puerto `8882`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

4. **análisis lingüístico del inglés** (puerto `8883`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

5. **análisis lingüístico del bable** (puerto `8884`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

6. **análisis lingüístico del gallego** (puerto `8885`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

7. **análisis lingüístico del italiano** (puerto `8886`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

8. **análisis lingüístico del portugués** (puerto `8887`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

9. **análisis lingüístico del ruso** (puerto `8888`), basado en [FreeLing](http://nlp.lsi.upc.edu/freeling/).

10. **análisis lingüístico del alemán** (puerto `8889`), basado en [`pattern`](http://www.clips.ua.ac.be/pages/pattern).

11. **análisis lingüístico del neerlandés** (puerto `8890`), basado en [`pattern`](http://www.clips.ua.ac.be/pages/pattern).
...
Para los servicios basados en pattern, hay un argumento opcional llamado para seleccionar un conjunto de etiquetas alternativo al [Penn Treebank](http://www.mozart-oz.org/mogul/doc/lager/brill-tagger/penn.html): [`STTS`](http://www.coli.uni-saarland.de/projects/sfb378/negra-corpus/stts.asc) para el alemán, [WOTAN](http://lands.let.ru.nl/literature/hvh.1999.2.ps) para el neerlandés: `tagset: stts|wotan|penn`.
...
Salida: *lista de hashes* `synsts`: *IDs de los synsets*, `palabra`: *palabra*, `lemas`: *lista de hashes* `categoria`: *etiqueta gramatical*, `lema`: *lema*
...
Salida: *lista de hashes* `palabra`: *palabra*, `entidades`: *lista de hashes* `categoria`: *etiqueta gramatical*, `lema`: *lema*
...
Salida: *lista de hashes* `expresión`: *expresión temporal/numérica*, `entidades`: *lista de hashes* `categoria: temporal|numero|partitivo|moneda|porcentaje|magnitud|numero`, `lema`: *expresión temporal/numércia formalizada*
...
Argumentos: `texto`: *texto de entrada*, `format`: `json|string` (servicios basados en `pattern`) o `json|string|fl` (servicios basados en `FreeLing`).

Para los servicios basados en pattern, hay un argumento opcional llamado para seleccionar un conjunto de etiquetas alternativo al [Penn Treebank](http://www.mozart-oz.org/mogul/doc/lager/brill-tagger/penn.html): [`STTS`](http://www.coli.uni-saarland.de/projects/sfb378/negra-corpus/stts.asc) para el alemán, [WOTAN](http://lands.let.ru.nl/literature/hvh.1999.2.ps) para el neerlandés: `tagset: stts|wotan|penn`.

Salida: Para la salida basada jerárquica basada en FreeLing (`fl`) o con paréntesis (`string`): *hash* `tree`: *cadena con formato jerárquico*
...
Para la salida en JSON: *lista de hashes* `tag`: *etiqueta*, `parent`: *etiqueta del nodo superior*, `level`: *nivel en la jerarquía*.

Cuando el nodo es una hoja sin hijos, el formato de salida es *lista de hashes* `tag`: *etiqueta*, `parent`: *etiqueta del nodo superior*, `level`: *nivel en la jerarquía* 
...
Salida: *lista de hashes* `parent`: *etiqueta del nodo superior*, `rel`: *tipo de dependencia*, `label`: *etiqueta del nodo*, `text`: *palabra*, `lemma`: *lema*, `tag`: *etiqueta morfológica de la palabra*.
